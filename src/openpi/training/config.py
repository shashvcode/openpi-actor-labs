...
class LeRobotSO100DataConfig(DataConfigFactory):
    """Config for SO-100 joystick control data. Actions are joystick commands (6 dims, already in [-1,1])."""
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[so100_policy.SO100Inputs(model_type=model_config.model_type)],
            outputs=[so100_policy.SO100Outputs()],
        )

        # No DeltaActions transform — joystick commands are instantaneous, not absolute positions.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotSO100DataConfig(
            repo_id="verm11/so100_joystick_pickup",
            base_config=DataConfig(prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        freeze_filter=pi0_config.Pi0Config(
    ...
    # v2: 500 episodes at 30 Hz, action_horizon=5
    TrainConfig(
        name="pi05_so100_lora_v2",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=6,
            action_horizon=5,
    ...
    # v3: 30Hz, action_horizon=11, 8 denoising steps at inference
    TrainConfig(
        name="pi05_so100_lora_v3",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=6,
            action_horizon=11,
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,  # pi05 base is trained with 32-dim actions; CANExcavatorOutputs slices to 8
            action_horizon=11,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ),
